...
{
    public final static String  nlnzFitsConfig = Fits.FITS_XML + "nlnz" + File.separator + "fits"
                                                       + File.separator;
    private boolean             enabled        = true;
    private final static Logger log            = LoggerFactory
                                                       .getLogger("edu.harvard.hul.ois.fits.tools.nlnz");

    public MetadataExtractor () throws FitsException
    {
        log.debug("Entering MetadataExtractor()");
        ...
        log.debug("Exiting MetadataExtractor()");
    }
    ...
    public ToolOutput extractInfo (File file) throws FitsToolException
    {
        ...
        // ByteArrayOutputStream tAdapterOutput = new
        ...
        try
        {
            ...
            // TransformProcessor.getInstance(outDTD, "nlnz_presmet.xsd");
            ...
            // Document tmpDom = saxBuilder.build(new StringReader(new
            // String(adapterByteOutput)));
            // XmlUtils.printToConsole(tmpDom);
            // transform adapter output byte array
            // transformer.transform(new
            // ByteArrayInputStream(adapterByteOutput),tAdapterOutput);
            // convert the transformed output to a dom object
            // InputSource is = new InputSource(new
            // ByteArrayInputStream(tAdapterOutput.toByteArray()));
            // is.setEncoding("UTF-8");
            dom = saxBuilder.build(new StringReader(adapterOutput.toString()));
            /*
             * //convert the adapter byte array output to a dom object is = new
             * InputSource(new ByteArrayInputStream(adapterByteOutput));
             * is.setEncoding("UTF-8"); try { rawDom = docBuilder.parse(is); }
        ...
        catch (Exception e)
        {
            ...
            throw new FitsToolException("NLNZ Metadata Extractor error while harvesting file "
                    + file.getName(), e);
        }
        finally
        {
            ...
            catch (IOException e)
            {
                throw new FitsToolException(
                        "Error closing NLNZ Metadata Extractor XML output stream", e);
            }
        }
    ...
    /*
     * public boolean isIdentityKnown(FileIdentity identity) { if(identity ==
     * null || identity.getMime() == null || identity.getMime().length() == 0 ||
     * identity.getFormat() == null || identity.getFormat().length() == 0) {
     * return false; } String format = identity.getFormat(); String mime =
     * identity.getMime(); if(format == null ||
     * mime.equalsIgnoreCase("file/unknown")) { return false; } else { return
     * true; } }
     */
