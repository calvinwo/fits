...
public abstract class CommandLine
{
    public static String exec (List<String> cmd, String directory) throws FitsToolCLIException
    {
        ...
        try
        {
            ...
            /*
             * StringBuffer sb = new StringBuffer(); BufferedReader bro = new
             * BufferedReader(new InputStreamReader(proc.getErrorStream()));
             * String line; while ((line = bro.readLine()) != null) {
             * sb.append(line); System.out.println(line); } // The process
             * should be done now, but wait to be sure. proc.waitFor();
             */
        ...
        finally
        {
            ...
            {
                throw new FitsToolCLIException("Error closing external command line output stream",
                        e);
            }
        }
        return output;
     ...
     * rt.exec(cmd.toString()); ProcessBuilder builder = new
     * ProcessBuilder(cmd); Process proc = builder.start(); StreamGobbler
     * errorGobbler = new StreamGobbler(proc.getErrorStream(),"ERROR", output);
     * StreamGobbler outputGobbler = new
     * StreamGobbler(proc.getInputStream(),"OUTPUT", output);
     * errorGobbler.start(); outputGobbler.start(); proc.waitFor(); }
     */
}