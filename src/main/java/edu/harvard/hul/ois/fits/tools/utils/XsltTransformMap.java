...
public class XsltTransformMap
{
    private final static Logger log                 = LoggerFactory
            .getLogger("edu.harvard.hul.ois.fits.tools.utils");
    public static Hashtable<String, String> getMap (String config)
            throws FitsConfigurationException
    {
        log.debug("Entering getMap(config={})", config);
        Hashtable<String, String> mappings = new Hashtable<String, String>();
        XMLConfiguration conf = null;
        try
        {
            File file = new File(XsltTransformMap.class.getClass().getResource(config).getFile());
        ...
        log.debug("Exiting getMap(): {}", mappings);
        return mappings;
    }
}
