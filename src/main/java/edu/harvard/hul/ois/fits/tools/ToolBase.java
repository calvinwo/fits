...
public abstract class ToolBase implements Tool
{
    ...
    {
        log.debug("Entering isIdentitiyKnown(identity={}", identity);
        if (!canIdentify())
        {
            log.debug("Exiting isIdentityKnown(): false");
            return false;
        }
        // identity and mimetype must not be null or empty strings for an
        // identity to be "known"
        if (identity == null || identity.getMime() == null || identity.getMime().length() == 0
        ...
        {
            log.debug("Exiting isIdentityKnown(): true");
            return true;
        }
    }
    ...
    public Document transform (String xslt, Document input) throws FitsToolException
    {
        ...
        {
            throw new FitsToolException(info.getName() + ": Error converting output using " + xslt,
                    e);
        }
        log.debug("Exiting transform(): {}", doc);
        return doc;
    }
    ...
    public void run ()
    {
        ...
        try
        {
            ...
            // " STARTING "+this.getClass());
            output = extractInfo(inputFile);
            // java.util.Date time2 = new java.util.Date();
            // System.out.println(new java.sql.Time(time2.getTime())
            // +" FINISHED "+this.getClass());
        }
