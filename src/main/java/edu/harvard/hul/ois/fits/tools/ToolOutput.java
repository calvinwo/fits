...
public class ToolOutput
{
    private final static Logger           log        = LoggerFactory
                                                             .getLogger("edu.harvard.hul.ois.fits.tools");
    private static DocumentBuilderFactory docBuilderFactory;
    {
        log.trace("Entering docBuilderFactory()");
        ...
        log.trace("Exiting docBuilderFactory()");
    }
    private static Namespace              ns         = Namespace.getNamespace("fits",
                                                             Fits.XML_NAMESPACE);
    ...
    {
        log.debug("Entering ToolOutput(tool={}, fitsXml={}, toolOutput={}", tool, fitsXml,
                toolOutput);
    ...
    {
        log.trace("Entering validateXmlOutput(output={})", output);
        try
        {
        ...
        log.trace("Exiting validateXmlOutput(): true");
        return true;
        /*
         ...
         * Fits.FITS_HOME+Fits.internalOutputSchema); XMLOutputter outputter2 =
         * new XMLOutputter(Format.getPrettyFormat()); try {
         * outputter2.output(output, System.out); } catch (IOException e) { //
         * TODO Auto-generated catch block e.printStackTrace(); } try {
         * XMLOutputter outputter = new XMLOutputter(); String xml =
         * outputter.outputString(output); builder.build(new InputSource(new
         * StringReader(xml))); } catch (Exception e) { return false; } return
         * true;
         */
    }
    ...
    {
        log.debug("Entering createFileIdentities(dom={}, info={})", dom, info);
        ...
        log.debug("Exiting createFileIdentities(): {}", identities);
        return identities;
    }
}
