...
public class Droid extends ToolBase
{
    ...
    public ToolOutput extractInfo (File file) throws FitsToolException
    {
        ...
        /*
         * List<FileIdentity> identities = new ArrayList(); for(int
         * i=0;i<idFile.getNumHits();i++) { FileFormatHit hit =
         * idFile.getHit(i); FileIdentity identity = new
         * FileIdentity(hit.getMimeType
         * (),hit.getFileFormatName(),hit.getFileFormatVersion()); //pronom id;
    ...
    {
        log.trace("Entering createXml(idFile={})", idFile);
        StringWriter out = new StringWriter();
        out.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        out.write("\n");
        out.write("<FileCollection xmlns=\"" + AnalysisController.FILE_COLLECTION_NS + "\">");
        out.write("\n");
        out.write("  <DROIDVersion>"
                + AnalysisController.getDROIDVersion().replaceAll("&", "&amp;") + "</DROIDVersion>");
        out.write("\n");
        out.write("  <SignatureFileVersion>" + droid.getSignatureFileVersion()
                + "</SignatureFileVersion>");
        out.write("\n");
        out.write("  <DateCreated>" + AnalysisController.writeXMLDate(new java.util.Date())
                + "</DateCreated>");
        out.write("\n");
        ...
        else
        {
            ...
            for (int hitCounter = 0; hitCounter < idFile.getNumHits(); hitCounter++)
            {
                ...
                out.write("\n");
                out.write("      <Name>" + formatHit.getFileFormatName().replaceAll("&", "&amp;")
                        + "</Name>");
                out.write("\n");
                if (formatHit.getFileFormatVersion() != null)
                {
                    out.write("      <Version>"
                            + formatHit.getFileFormatVersion().replaceAll("&", "&amp;")
                            + "</Version>");
                    out.write("\n");
                }
                ...
                {
                    out.write("      <PUID>"
                            + formatHit.getFileFormatPUID().replaceAll("&", "&amp;") + "</PUID>");
                    out.write("\n");
                }
                if (formatHit.getMimeType() != null)
                {
                    out.write("      <MimeType>" + formatHit.getMimeType().replaceAll("&", "&amp;")
                            + "</MimeType>");
                    out.write("\n");
                }
                if (!"".equals(formatHit.getHitWarning()))
                {
                    out.write("      <IdentificationWarning>"
                            + formatHit.getHitWarning().replaceAll("&", "&amp;")
