...
{
    private static Namespace          xsiNS              = Namespace
                                                                 .getNamespace("xsi",
    ...
    static private final List<String> repeatableElements = new ArrayList<String>(
    ...
    {
        log.debug("Entering OISConsolidator()");
        ...
        log.debug("Exiting OISConsolidator()");
    ...
    {
        Object[] obj = { results, xpath_query, useChildren };
        log.trace("Entering findAnElement(results={}, xpath_query={}, useChildren={}", obj);
                ...
                if (dom != null)
                {
                    ...
                    if (e != null && e.getChildren().size() > 0)
                    {
                        ...
                        if (children.size() > 0)
                        {
                            ...
                            log.trace("Exiting findAnElement(): {}", child);
                            return child;
                        }
                    }
                }
        ...
        log.trace("Exiting findAnElement(): null");
        return null;
    }
    ...
    {
        log.trace("Entering cullResults(results={})", results);
        ...
        for (ToolOutput result : results)
        {
            if (result != null)
            {
                ...
                // if the tool can't identify files, or if it can and all
                // identities are good
        ...
        log.trace("Exiting cullResults(): {}", newResults);
        return newResults;
    }

    /** Skips null identities and returns the first tools unknown Identities that
     * are found
     * 
     * @param results
     * @return */
    ...
    {
        log.trace("Entering getFirstUnknownIdentity(results={})", results);
        ...
        for (ToolOutput result : results)
        {
            ...
            if (result.getTool().canIdentify() && toolIdentities != null
                    && toolIdentities.size() > 0)
            {
        ...
        log.trace("Exiting getFirstUnknownIdentity(): {}", identities);
        return identities;
    }
    ...
    {
        log.trace("Entering getFirstPartialIdentity(results={})", results);
        List<FitsIdentity> identities = new ArrayList<FitsIdentity>();
        for (ToolOutput result : results)
        {
            // if result is null skip it
        ...
        log.trace("Exiting getFirstPartialIdentity(): {}", identities);
        return identities;
    }

    private boolean allIdentitiesAreGood (ToolOutput result)
    {
        log.trace("Entering allIdentitiesAreGood(result={}", result);
        List<ToolIdentity> identities = result.getFileIdentity();
        if (identities.size() == 0)
        {
            log.trace("Exiting allIdentitiesAreGood(): false");
            return false;
        }
        Tool t = result.getTool();
        ...
        log.trace("Exiting allIdentitiesAreGood(): true");
        return true;
    }

    private List<Element> mergeXmlResults (List<ToolOutput> results, Element element)
    {
        log.trace("Entering mergeXmlResults(results={}, element={}", results, element);
        ...
        {
            log.trace("Exiting mergeXmlResults(): {}", fitsElements);
            return fitsElements;
        }
        ...
        else if (equalityResult == CONFLICT && reportConflicts
                && !isRepeatableElement(fitsElements))
        {
        ...
        log.trace("Exiting mergeXmlResults(): {}", consolidatedElements);
        return consolidatedElements;
    }

    private boolean isRepeatableElement (List<Element> fitsElements)
    {
        log.trace("Entering isRepeatableElement(fitsElement={})", fitsElements);
        String name = fitsElements.get(0).getName();
        if (repeatableElements.contains(name))
        {
            log.trace("Exiting isRepeatableElement(): true");
            return true;
        }
        else
        {
            log.trace("Exiting isRepeatableElement(): false");
            return false;
        }
    }
    ...
    {
        log.trace("Entering consolidateFitsElements(fitsElements={})", fitsElements);
        ...
        log.trace("Exiting consolidateFitsElements(): {}", conElements);
        return conElements;
    }

    private synchronized List<Element> removeUnknowns (List<Element> fitsElements)
    {
        log.trace("Entering removeUnknowns(fitsElements={})", fitsElements);
        ...
        log.trace("Exiting removeUnknowns(): {}", fitsElements);
        return fitsElements;
    }

    private int testEquality (List<Element> fitsElements)
    {
        log.trace("Entering testEquality(fitsElements={})", fitsElements);
        if (fitsElements.size() == 1)
        {
            log.trace("Exiting testEquality(): {}", SINGLE_RESULT);
        ...
        for (int i = 1; i < fitsElements.size(); i++)
        {
            ...
            else if (eText.matches(REAL_NUMBER) && ee.getText().matches(REAL_NUMBER))
            {
                ...
                {
                    log.trace("Exiting testEquality(): {}", CONFLICT);
                    return CONFLICT;
                }
            }
            else
        ...
        log.trace("Exiting testEquality(): {}", ALL_AGREE);
        return ALL_AGREE;
    }
    ...
    {
        log.trace("Entering parentContainsChild(parent={}, childName={}", parent, childName);
        List<Element> children = parent.getChildren();
        for (Element e : children)
        {
            if (e.getName().equalsIgnoreCase(childName))
            {
                log.trace("Exiting parentContainsChild(): true");
                return true;
            }
        }
        log.trace("Exiting parentContainsChild(): false");
        return false;
    }

    private List<ToolIdentity> getAllIdentities (List<ToolOutput> results)
    {
        log.trace("Entering getAllIdentities(results={})", results);
    ...
    {
        log.trace("Entering mergeExternalIdentifiers(a={}, b={})", a, b);
        // add external identifiers to consolidated element from the new item,
    ...
    private List<FitsIdentity> consolidateIdentities (List<ToolIdentity> identities)
    {
        ...
        for (ToolIdentity ident : identities)
        {
            ...
            while (iter.hasNext())
            {
                ...
                // if identities don't match check format tree for
                // specific/generic formats
                else
                {
                    int matchCondition = checkFormatTree(ident, identitySection);
                    if (matchCondition == -1)
                    {
                        // ident is more specific. Most specific identity should
                        // always
                    ...
                    else if (matchCondition == 1)
                    {
                        ...
                        // do nothing, keep going and add to consolidated
                        // identities if no other matches
                    }
                    // formats are not specific or generic version of one
                    // another
                    else
                    {
                        // leave anyMatches as false so ident is added to
                        // consolidated identities
                    }
                }
            }
            // if there were no matches to existing consolidated identities add
            // it
        ...
        log.trace("Exiting consolidatedIdentities(): {}", consolidatedIdentities);
        return consolidatedIdentities;
    }

    /** Returns 1 if a is more specific than b. Returns -1 if b is more specific
     * than a. If neither is more specific then 0 is returned.
     * 
     * @param a
     * @param b
     * @return */
    ...
    {
        log.trace("Entering checkFormatTree(a={}, b={})", a, b);
        ...
        if (a_element != null && b_element != null)
        {
            ...
            {
                log.trace("Exiting checkFormatTree(): 1");
                return 1;
            }
            ...
            {
                log.trace("Exiting checkFormatTree(): 0");
                return 0;
            }
        }
        log.trace("Exiting checkFormatTree(): 0");
    ...
    {
        log.trace("Entering filterToolOutput(section={}, results={})", section, results);
        ListIterator<ToolOutput> iter = results.listIterator();
        while (iter.hasNext())
        {
            ToolOutput result = iter.next();
            // if the identity section doesn't contain the results from the tool
            // remove it
        ...
        log.trace("Exiting filterToolOutput()");
    }

    /*
     * (non-Javadoc)
     * @see
     * edu.harvard.hul.ois.fits.DataConsolidator#processResults(java.util.List)
     */
    public FitsOutput processResults (List<ToolOutput> results)
    {
        log.debug("Entering processResults(results={})", results);
        // Remove any null results, or results from tools that have the
        // capability to identify files,
        ...
        fits.setAttribute(new Attribute("schemaLocation", Fits.XML_NAMESPACE + " "
                + Fits.externalOutputSchema, xsiNS));
        ...
        for (FitsIdentity identSection : identitySections)
        {
            ...
            // tools agree
            boolean toolsAgree = (identitySections.size() == 1 && identSection.getReportingTools()
                    .size() > 1);
            // add format version
            ...
            // if conflicts are being reported continue, else break after 1
            // iteration
            if (reportConflicts)
            {
                continue;
        ...
        // check fileinfo, do normal xml comparison. Use all non-culled tool
        // output
        ...
        // if child.getParent() !exist in mergedDoc, then create and add these
        // elements to it.
        ...
        /*
         * Consolidate results from each tool Check for identical and unknown
         * values for each field Check format tree for specific/generic formats
         * if conflicts exist && configured to report multiple values in
         * conflict report both values else pick value using preferred tool list
         * ordering while non empty results exist - check for elements in
         * //identity, //info and //metadata (method for this) Start with first
         * result that is not empty (method for this to grab any element from
         * these sections) pull out an element /fits/summary/identity
         * /fits/summary/info /fits/summary/metadata look up and pull that
         * element out of other results do comparison Identical? unknown values?
         * consult tree? configured to report multiple conflicting values? use
         * preferred ordering of tools add result or results if configured to
         * report multiples when conflict exists to mergedDoc continue with next
         * element until result is empty continue with all results until all are
         * empty.
         */
        ...
        log.debug("Exiting processResults(): {}", results);
        return result;
    }

    private boolean isPartialIdentity (List<ToolIdentity> identities)
    {
        log.trace("Entering isPartialIdentity(identities={}", identities);
        ...
        if (identity != null)
        {
            ...
            if ((validMime && validFormat) || (validMime && !validFormat)
                    || (validFormat && !validMime))
            {
                if ((mime.equals("application/octet-stream") && !format.equals("Unknown Binary"))
                        || (format.equals("Unknown Binary") && !mime
                                .equals("application/octet-stream")))
                {
                    log.trace("Exiting isPartialIdentities(): true");
                    return true;
                }
            }
        }
        log.trace("Exiting isPartialIdentities(): false");
        return false;
    }
}
