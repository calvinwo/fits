...
public class Fits
{
    ...
    public static int               maxThreads       = 20;                                                  // GDM
                                                                                                             // 16-Nov-2012
    ...
    public Fits (String fits_home) throws FitsConfigurationException
    {
        ...
        if (FITS_HOME == null)
        {
            // if env variable not set check for fits_home passed into
            // constructor
            if (fits_home != null)
            {
                FITS_HOME = fits_home;
            ...
            XMLStreamException
    {
        ...
        if (cmd.hasOption("i"))
        {
            ...
            else
            {
                ...
                fits.outputResults(result, cmd.getOptionValue("o"), cmd.hasOption("x"),
                        cmd.hasOption("xc"), false);
            }
        }
        else
        {
    ...
    private void doDirectory (File inputDir, File outputDir, boolean useStandardSchemas,
            boolean standardCombinedFormat) throws FitsException, XMLStreamException, IOException
    {
        log.trace("Entering doDirectory(inputDir={}, outputDir={}", inputDir, outputDir);
        for (File f : inputDir.listFiles())
        {
            ...
            else if (f.isFile())
            {
                ...
                String outputFile = outputDir.getPath() + File.separator + f.getName()
                        + ".fits.xml";
     ...
     * @param useStandardSchemas - use standard schemas if available for output
     *            type
     * @throws FitsException
     * @throws XMLStreamException
     * @throws IOException */
    private FitsOutput doSingleFile (File inputFile) throws FitsException, XMLStreamException,
            IOException
    {
        log.trace("Entering doSingleFile(inputFile={})", inputFile);
        ...
        log.trace("Exiting doSingleFile(): {}", result);
        return result;
    }

    /** Outputs FitsOutput to an XML document at outputLocation.
     * 
     * @param result
     * @param outputLocation
     * @param standardSchema
     * @param standardCombinedFormat
     * @param dirMode
     * @throws XMLStreamException
     * @throws IOException
     * @throws FitsException */
    private void outputResults (FitsOutput result, String outputLocation, boolean standardSchema,
            boolean standardCombinedFormat, boolean dirMode) throws XMLStreamException,
            IOException, FitsException
    {
        log.trace("Entering outputResults(result={}, outputLocation={}", result, outputLocation);
        ...
        try
        {
            ...
            {
                throw new FitsException(
                        "The output location must be provided when running FITS in directory mode");
            }
            // if -x is set, then convert to standard metadata schema and output
            // to -o
            if (standardSchema)
            {
        ...
        finally
        {
            ...
            log.trace("Exiting outputResults()");
        }
    }

    /** Output the XML in standard combined format.
     * 
     * @param result
     * @param out
     * @throws XMLStreamException
    ...
    {
        log.debug("Entering outputStandardCombinedFormat(result={}, out={}", result, out);
        ...
        log.debug("Exiting outputStandardCombinedFormat()");
    }

    /** Output the XML in standard schema.
     * 
     * @param fitsOutput
     * @param out
     * @throws XMLStreamException
            ...
            throws XMLStreamException, IOException
    {
        ...
        if (xml != null && transformer != null)
        {
            ...
            try
            {
                ...
                xml.output(sw);
                // convert output stream to byte array and read back in as
                // inputstream
                Source source = new StreamSource(new ByteArrayInputStream(
                        xmlOutStream.toByteArray()));
            ...
            finally
            {
                xmlOutStream.close();
                xsltOutStream.close();
                log.trace("Exiting outputStandardSchemaXml()");
            }
        }
        else
        {
            System.err
                    .println("Error: output cannot be converted to a standard schema format for this file");
            log.debug("Exiting outputStandardSchemaXml");
        }
    }
    ...
    /*
     * ORIGINAL EXAMINE METHOD WITHOUT THREADS public FitsOutput
     * examineOriginal(File input) throws FitsException { if(!input.exists()) {
     * throw new
     * FitsConfigurationException(input+" does not exist or is not readable"); }
     * List<ToolOutput> toolResults = new ArrayList<ToolOutput>(); //run file
     * through each tool, catching exceptions thrown by tools List<Exception>
     * caughtExceptions = new ArrayList<Exception>(); String path =
     * input.getPath().toLowerCase(); String ext =
     * path.substring(path.lastIndexOf(".")+1); for(Tool t :
     * toolbelt.getTools()) { if(t.isEnabled()) {
     * if(!t.hasExcludedExtension(ext)) { try { ToolOutput tOutput =
     * t.extractInfo(input); toolResults.add(tOutput); } catch(Exception e) {
     * caughtExceptions.add(e); } } } } // consolidate the results into a single
     * DOM FitsOutput result = consolidator.processResults(toolResults);
     * result.setCaughtExceptions(caughtExceptions); for(Tool t:
     * toolbelt.getTools()) { t.resetOutput(); } return result; }
     */
    /** Takes in a file and runs the tool to get information on it to put into a
     * FitsOutput object.
     * 
     * @param input
     * @return
    ...
    {
        log.debug("Entering examine(input={})", input);
        ...
        for (Tool t : toolbelt.getTools())
        {
            if (t.isEnabled())
            {
                // figure out of the tool should be run against the file
                // depending on the include and exclude extension lists
                RunStatus runStatus = RunStatus.SHOULDNOTRUN;
                // if the tool has an include-exts list and it has the extension
                // in it, then run
                ...
                // if the tool does not have an include-exts or exclude-exts
                // list then run
        ...
        log.debug("Exiting examine(): {}", result);
        return result;
    }

    /** Returns ToolBelt object.
     * 
     * @return ToolBelt */
    ...
    {
        log.trace("Entering countActiveTools={}", threads);
        int count = 0;
        ...
        log.trace("Exiting countActiveTools(): {}", count);
        return count;
    }
}
