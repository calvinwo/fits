...
{
    /** Returns the first value of the element in the provided dom object.
    ...
    /** concatenates the values of all elements children into a single string and
     * returns it
     * 
     * @param dom
     * @param element
     ...
     * e.printStackTrace(); } Document dom = docBuilder.newDocument(); Element
     * element = dom.createElement(elementName); CDATASection cdata =
     * dom.createCDATASection(text); element.appendChild(cdata);
     * dom.appendChild(element); return dom; }
     */
    /*
     * public static void printToConsole(Document dom) { XMLOutputter outputter
     * = new XMLOutputter(); try { outputter.output(dom, System.out); } catch
     * (IOException e) { e.printStackTrace(); } }
     */
    /*
     * public static Element getChildWithAttribute(Element parent, String
     * attName, String attValue) { for(Element
     * e:(List<Element>)parent.getChildren()) { Attribute attr =
     * e.getAttribute(attName); if(attr != null &&
     * attr.getValue().equalsIgnoreCase(attValue)) { return e; } } return null;
     * }
     */
    @SuppressWarnings("unchecked")
