...
public class FileUtility extends ToolBase
{
    ...
    private List<String>        WIN_COMMAND       = new ArrayList<String>(
                                                          Arrays.asList(Fits.FITS_TOOLS
                                                                  + "file_utility_windows/bin/file.exe"));
    private List<String>        UNIX_COMMAND      = new ArrayList<String>(Arrays.asList("file"));
    private List<String>        FILE_TEST_COMMAND = new ArrayList<String>(Arrays.asList("which",
                                                          "file"));
    private final static String WIN_FILE_DATE     = "6/7/2008";
    private boolean             enabled           = true;
    public final static String  xslt              = Fits.FITS_XML
                                                          + "fileutility/fileutility_to_fits.xslt";
    private final static Logger log               = LoggerFactory
                                                          .getLogger("edu.harvard.hul.ois.fits.tools.fileutility");

    public FileUtility () throws FitsToolException
    {
        log.debug("Entering FileUtility()");
    ...
    public ToolOutput extractInfo (File file) throws FitsToolException
    {
        ...
        if (execMimeOut.startsWith("text/") && execMimeOut.contains("charset="))
        {
             ...
             * execOut.contains("ISO-8859")) { format = "Plain text"; }
             */
            format = "Plain text";
    ...
    {
        log.debug("Entering testOSForCommand()");
        String output = CommandLine.exec(FILE_TEST_COMMAND, null);
        if (output == null || output.length() == 0)
        {
            log.debug("Exiting testOSForCommand(): false");
            return false;
        }
        else
        {
            log.debug("Exiting testOSForCommand(): true");
            return true;
        }
    }

    private Document createXml (String mime_s, String format_s, String charset_s,
            List<String> linebreaks, String rawOutput_s) throws FitsToolException
    {
        log.trace(
                "Entering createXml(mime_s={}, format_s={}, charset_s={}, linebreaks={}, rawOutput_s)",
                mime_s, format_s, charset_s, linebreaks, rawOutput_s);
        // xml root
    ...
    /*
     * public boolean isIdentityKnown(FileIdentity identity) { //identity and
     * mimetype must not be null or empty strings for an identity to be "known"
     * if(identity == null || identity.getMime() == null ||
     * identity.getMime().length() == 0 || identity.getFormat() == null ||
     * identity.getFormat().length() == 0) { return false; } String format =
     * identity.getFormat(); String mime = identity.getMime();
     * if(format.equals("data") || format.equals("Unknown Binary") ||
     * mime.equals("application/octet-stream")) { return false; } else { return
     * true; } }
     */
