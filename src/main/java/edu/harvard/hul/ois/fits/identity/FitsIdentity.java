...
public class FitsIdentity
{
    ...
    /** Adds a ToolInfo object to the reported tools list only if one with the
     * same name and version has not already been added.
     * 
     * @param info */
    public void addReportingTool (ToolInfo info)
    {
        for (ToolInfo reportedInfo : reportingTools)
        {
            ...
            if (Name.equalsIgnoreCase(info.getName())
                    && version.equalsIgnoreCase(info.getVersion()))
            {
                return;
            }
        }
