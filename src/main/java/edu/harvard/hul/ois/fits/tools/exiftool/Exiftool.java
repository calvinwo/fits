...
public class Exiftool extends ToolBase
{
    ...
    private List<String>        winCommand         = new ArrayList<String>(
                                                           Arrays.asList(Fits.FITS_TOOLS
                                                                   + "exiftool/windows/exiftool.exe"));
    private List<String>        unixCommand        = new ArrayList<String>(Arrays.asList("perl",
                                                           Fits.FITS_TOOLS
    ...
    private final static Logger log                = LoggerFactory
                                                           .getLogger("edu.harvard.hul.ois.fits.tools.exiftool");

    public Exiftool () throws FitsException
    {
        log.debug("Entering ExifTool()");
    ...
    public ToolOutput extractInfo (File file) throws FitsToolException
    {
        ...
        /*
         * Document exifDoc = null; try { exifDoc = saxBuilder.build(new
         * StringReader(execOut)); } catch (Exception e) { throw new
         * FitsToolException("Error parsing ffident XML Output",e); } String
         * format = XmlUtils.getDomValue(exifDoc.getDocument(),"File:FileType");
         ...
         * ); String test =
         * exifDoc.getRootElement().getChildText("rdf:Description",ns);
         */
    ...
    {
        log.trace("Entering testOSForPerl()");
        String output = CommandLine.exec(perlTestCommand, null);
        if (output == null || output.length() == 0)
        {
            log.debug("Exiting testOSForPerl(): false");
            return false;
        }
        else
        {
            log.debug("Exiting testOSForPerl(): true");
            return true;
        }
    }
    ...
    {
        log.trace("Entering createXml(execOut={})", execOut);
     ...
     * identity.getFormat().length() == 0) { return false; } String mime =
     * identity.getMime(); if(mime.equals("application/unknown")) { return
