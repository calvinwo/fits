...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="j2id"
		select="//j2:feature[@name='PresumptiveFormats']//j2:feature[@name='JHOVE2Identifier']/j2:value" />
	<xsl:variable name="confidence"
		select="//j2:feature[@name='PresumptiveFormats']//j2:feature[@name='Confidence']/j2:value" />
	...
	<xsl:template match="/">
		...
		<identification>
			<identity>
				<xsl:attribute name="mimetype">
					<xsl:choose>
						<xsl:when test="$j2id='[JHOVE2] http://jhove2.org/terms/format/zip'">
		...
		<filestatus>
			<well-formed toolname="$toolname" toolversion="$toolversion"
			...
			<valid toolname="$toolname" toolversion="$toolversion" status="SINGLE_RESULT">
				<xsl:choose>
					<xsl:when
						test="$confidence='PositiveSpecific' or $confidence='PositiveGeneric' or $confidence='Validated'">
						true
